...
def evalTSP(individual):
    ...
    for i in range(1, N_POINTS):
        distance_calc += distances[individual[i - 1]][individual[i]]
    # Add distance from the last point back to the central node C
    distance_calc += distances[individual[-1]][0]
...
def main():
    ...
    print("Best 5 individuals with distances\n")
    mapping = {num: f'E{num}' if num > 0 else 'C' for num in range(100)}

    for i, path in enumerate(converted_hof):
        distances_on_path = []
        total_distance = 0
        for j in range(1, len(path)):
            current_node = int(path[j - 1][1:])
            next_node = int(path[j][1:])
            distance = distances[current_node][next_node]
            distances_on_path.append(distance)
            total_distance += distance
        
        # Add distance from C to the first node and last node back to C
        total_distance += distances[0][int(path[1][1:])]
        total_distance += distances[int(path[-2][1:])][0]
        distances_on_path.insert(0, distances[0][int(path[1][1:])])
        distances_on_path.append(distances[int(path[-2][1:])][0])
        
        print("Sequence", i, ": Distance", round(total_distance, 1))
        print("Distances on path:", distances_on_path)
